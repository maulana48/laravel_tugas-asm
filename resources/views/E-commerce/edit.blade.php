...
@section('content')
	...
	<div class="relative min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8 bg-gray-500 bg-no-repeat bg-cover relative items-center"
		...
		<div class="sm:max-w-lg w-full p-10 bg-white rounded-xl z-10">
			...
			<form class="mt-8 space-y-3" action="{{ route('ec.update', $barang->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
						<div class="grid grid-cols-1 space-y-2">
							<label class="text-sm font-bold text-gray-500 tracking-wide">Nama</label>
								<input class="text-base p-2 border border-gray-300 rounded-lg focus:outline-none focus:border-indigo-500" type="" placeholder="Masukkan nama produk" @error('nama') is-invalid @enderror" name="nama" id="nama" required autofocus value="{{ old('nama', $barang->nama) }}">
						...
						<div class="grid grid-cols-1 space-y-2">
							<label class="text-sm font-bold text-gray-500 tracking-wide">Harga</label>
								<input class="text-base p-2 border border-gray-300 rounded-lg focus:outline-none focus:border-indigo-500" type="number" placeholder="Masukkan harga produk" @error('harga') is-invalid @enderror name="harga" id="harga" required autofocus value="{{ old('harga', $barang->harga) }}"">
						...
						<div class="grid grid-cols-1 space-y-2">
							<label class="text-sm font-bold text-gray-500 tracking-wide">Diskon</label>
								<input class="text-base p-2 border border-gray-300 rounded-lg focus:outline-none focus:border-indigo-500" type="" placeholder="Masukkan diskon produk" @error('diskon') is-invalid @enderror form-siswa name="diskon" id="diskon" autofocus value="{{ old('diskon', $barang->diskon) }}">
