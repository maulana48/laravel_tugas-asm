    ...
    <div class="container mx-auto flex flex-wrap py-6">
        ...
        <section class="w-full md:w-2/3 flex flex-col items-center px-3">
                ...
                <div class="bg-white flex flex-col justify-start p-6">
                    <div class="flex justify-between">
                        ...
                        <div class="flex gap-2">
                            <a href="{{ route('blog.edit', $post->id) }}" class="btn bg-yellow-700 hover:bg-yellow-500 py-2 px-5 text-white rounded-xl">Edit Post</a>
                    ...
                    <p href="#" class="text-sm pb-8">
                        By <a href="#" class="font-semibold hover:text-gray-800">{{ $user->name }}</a>, Published on {{ $post->created_at->format('F jS, Y') }}
                    </p>
            ...
            <div class="w-full flex pt-6">
                @if ($PVpost)
                    <a href="{{ route('blog.show', $PVpost->id) }}" class="w-1/2 bg-white shadow hover:shadow-md text-left p-6">
                        <p class="text-lg text-blue-800 font-bold flex items-center"><i class="fas fa-arrow-left pr-1"></i> Previous</p>
                        <p class="pt-2">{{ $PVpost->title }}</p>
                    </a>
                @endif
                @if ($NXpost)
                    <a href="{{ route('blog.show', $NXpost->id) }}" class="w-1/2 bg-white shadow hover:shadow-md text-right p-6">
        ...
        <div class="mx-auto pt-10">
            ...
            @foreach ($latest as $l)
                    <div class=" overflow-hidden shadow-lg transition duration-500 ease-in-out transform hover:shadow-2xl rounded-lg md:w-80">
                        @if ($l->image)
                            <img alt="blog photo" src="{{ asset($l->image) }}" class="max-h-40 w-full object-cover" />
                        @else
                        ...
                        <div class="bg-white w-full p-4">
                            <a href="#" class="text-blue-600 text-2xl font-medium">{{ $l->title }}</a>
                            <p class="text-gray-600 font-light text-md">
                                {{ $l->excerpt }}
                                <a class="inline-flex text-blue-600" href="{{ route('blog.show', $l->id) }}">Read More</a>
                            </p>
